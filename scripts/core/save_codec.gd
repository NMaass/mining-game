...
##
## Migration is ordered + forward-only along a v0→v1→v2 chain. The LOAD-BEARING step is v0→v1: a
...
## stamps the version + introduces the `settings` block; normalization also defaults a missing
## settings block, so v1→v2 is forward-declaration + belt-and-suspenders rather than the sole
## mechanism. Unknown fields are ignored; missing fields default (AC-5.11.2).
...
static func _default_settings() -> Dictionary:
	return {"sfx_volume": 0.8, "music_volume": 0.6, "motion_intensity": 0.3, "text_scale": 1.0}
...
## re-applied by SettingsState.from_state on load). Missing keys fall back to the neutral default —
## so a hand-edited or partial save can't inject an out-of-range setting.
