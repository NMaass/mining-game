...
> Reconciled with **SPEC.md v0.5.0** (2026-06-15 design pivot). v0.5 moves from an infinite vertical
> shaft to a **bounded rectangular mine volume** per dig, adds a **prestige offer UI** when the relic
> block breaks (accept → bank +1 point and end dig; decline → keep digging), and adds a **HUD
> resource-odds readout** at the current depth. Prestige is **exactly 1 point per relic**; money is
...
> remains the primary look/feel reference; Dome Keeper's bounded-mine structure is an adjacent
> reference. When this file and `SPEC.md`/`CLAUDE.md` disagree, the spec wins. (`CLAUDE.md` is the
> v0.5.0 fast path; keep it and this file in sync.)
...
- **One block grid.** The mine is a single chunked `TileMapLayer` of typed square blocks inside a
  bounded rectangular volume (width × depth in cells from `/data`), with a sliding window of resident
  chunks (recycle off-screen) so resident cell count stays bounded. Generation sets block types (pure
  function of saved seed + absolute cell). Destruction removes blocks in a radius by a grid walk over
  the per-cell HP array (no physics queries, no full-map scan). Do not implement marching squares,
  value-density fields, or smooth contours — squares only.
